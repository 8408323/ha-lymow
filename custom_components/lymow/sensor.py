...
class LymowErrorSensor(LymowSensor):
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        if warning_codes is not None:
            ...
            attrs["warning_descriptions"] = [WARNING_DESCRIPTIONS.get(int(w), f"Unknown ({w})") for w in warning_codes]
        all_error_codes = data.get("errorCodes")
        if all_error_codes is not None:
            attrs["error_codes"] = all_error_codes
            attrs["error_descriptions"] = [ERROR_DESCRIPTIONS.get(int(e), f"Unknown ({e})") for e in all_error_codes]
        return attrs
