...
_MAP_CONTENT_GLOBAL_ZONE_CONFIG = 11
_MAP_CONTENT_GLOBAL_CHANNEL_CONFIG = 12
...
def decode_map_response(pb_bytes: bytes) -> dict[str, Any]:
    ...
    # ---- Global mowing settings PbZoneConfig (f11) — echo of the last SET_TASK_CONFIG.
    gzc_raw = _first(content, _MAP_CONTENT_GLOBAL_ZONE_CONFIG)
    if isinstance(gzc_raw, bytes):
        result["mowingSettings"] = decode_global_zone_config(gzc_raw)

    # ---- Global channel config (f12) — channel lift and mode defaults.
    gcc_raw = _first(content, _MAP_CONTENT_GLOBAL_CHANNEL_CONFIG)
    if isinstance(gcc_raw, bytes):
        result["channelConfig"] = decode_global_channel_config(gcc_raw)
...
def decode_global_zone_config(data: bytes) -> dict[str, Any]:
    """Decode PbMap.f11 globalZoneConfig — the robot's current global mowing settings.

    Field layout confirmed from live query_map response (2026-05-27).
    Field numbers align with _TASK_CONFIG_FIELDS encoder:
      f1  cutHeight (mm, int32)
      f4  moveSpeed (float32 stored as varint bits)
      f5  brushSpeed (int32, 0-4)
      f6  cutSpeed (int32, 0-4)
      f7  cleanMode (int32: 0=parallel, 1=spiral, 2=random)
      f8  enabledZoneMask (uint64 bitmask — all-ones = all enabled)
      f9  pathSpacing (mm, int32)
      f10 perimeterMowLaps (int32)
      f11 perimeterMowDir (int32: 0=CW, 1=CCW, 2=random)
      f12 noGoMowLaps (int32)
      f13 obsDecMode (int32: 0=off, 1=slow, 2=detour)
      f14 pathOrder (bool: 0=normal, 1=reverse)
      f15 startProgress (int32)
      f16 relativeCleanDir (degrees, int32)
      f17 lineFollowMode (bool)
      f18 disableOuterDischarge (bool)
      f19 followDetectMode (int32)
    """
    f = _decode_fields(data)
    out: dict[str, Any] = {}
    ...
    for key, field_no in (
        ("pathOrder", 14),
        ("lineFollowMode", 17),
        ("disableOuterDischarge", 18),
    ):
        val = _first(f, field_no)
        if val is not None:
            out[key] = bool(val)

    # f4 moveSpeed and f5 brushSpeed are stored as float32 bits in a varint field
    for key, field_no in (("moveSpeed", 4), ("brushSpeed", 5)):
        raw = _first(f, field_no)
        if raw is not None:
            import struct as _struct

            out[key] = round(float(_struct.unpack("<f", _struct.pack("<I", raw & 0xFFFFFFFF))[0]), 4)

    return out


def decode_global_channel_config(data: bytes) -> dict[str, Any]:
    """Decode PbMap.f12 globalChannelConfig.

    Field layout from live response (2026-05-27):
      f1 channelMode (int32)
      f2 channelLiftHeight (int32, mm)
      f3 reserved (int32)
    """
    f = _decode_fields(data)
    out: dict[str, Any] = {}
    for key, field_no in (("channelMode", 1), ("channelLiftHeight", 2)):
        val = _first(f, field_no)
        if val is not None:
            out[key] = val
    return out
