"""Wire-format snapshot tests for protocol.py.

What these catch: silent field-number / wire-type drift. The existing
test_protocol.py builds payloads with the same `_field_*` helpers the
production encoders use, then decodes them with the same `_decode_fields`.
A bug that systematically renames "field 13" to "field 14" in both encoder
and decoder is self-consistent and passes those tests. These snapshot tests
pin the actual byte output of each public encoder to a frozen literal — any
field-number, tag, or wire-type change shows up as a byte diff.

How to update: if a snapshot fails for a *legitimate* protocol change, run
the encoder under uv, paste the new hex into the constant, and document the
why in the commit message. Don't blindly regenerate — the whole point is to
make wire-format drift visible.

Provenance: the hex literals were generated by running the public encoders
in this codebase against the inputs shown in each test. Field numbers /
wire types in the literals trace back to:
  - Hermes RE notes in protocol.py docstrings
  - reference/apk_hermes_re memory + reference/robotconfig_wire memory
"""
